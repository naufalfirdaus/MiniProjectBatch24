...
const updateTraineeEvaluationReview = async (payload: any) => {
  try {
    const result = await axios.put(`${config.domain}/api/bootcamp/batch/evaluation/review?userentityid=${payload.userId}`, payload.data);
    return result.data;
  } catch (error) {
    return error;
  }
