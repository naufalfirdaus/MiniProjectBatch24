...
export default function EditBatch(){
    ...
    const formik = useFormik({
        ...
        initialValues: {
           batchEntityId: Object.keys(batch).length != 0 && batch.batchEntityId,
           batchName: Object.keys(batch).length != 0 && batch.batchName,
           batchStartDate: Object.keys(batch).length != 0 && batch.batchStartDate.toString().slice(0,10),
           batchEndDate: Object.keys(batch).length != 0 && batch.batchEndDate.toString().slice(0,10),
           batchInstructorId: Object.keys(batch).length != 0 && batch.instructorPrograms[0].inproEmpEntityId,
           batchCoInstructorId: Object.keys(batch).length != 0 && batch.instructorPrograms[1].inproEmpEntityId,
        },
        onSubmit: (values) => {
    ...
    const handleMemberButton = (e: any, id: number, status: string) => {
        e.preventDefault();
        const selectedCandidate = candidatesProgram.find((candidate: any) => candidate.prapUserEntityId == id);
        
        if(status == 'add'){
            setMembers([...members, { idUser: selectedCandidate.prapUserEntityId, name: `${selectedCandidate.prapUserEntity.userFirstName} ${selectedCandidate.prapUserEntity.userLastName}` }]);
        } else {
            const removedMember = members.filter((member: any) => member.idUser !== selectedCandidate.prapUserEntityId);
                    ...
                    <form onSubmit={formik.handleSubmit}>
                        <div className="grid md:grid-cols-3 gap-4">
                            ...
                            <div className="mb-6">
                                ...
                                <select id="batchEntityId" name="batchEntityId" value={formik.values.batchEntityId} onChange={formik.handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 w-full">
                            ...
                            <div className="mb-6">
                                ...
                                <select id="batchInstructorId" name="batchInstructorId" value={formik.values.batchInstructorId} onChange={formik.handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 w-full">
                            ...
                            <div className="mb-6">
                                ...
                                <select id="batchCoInstructorId" name="batchCoInstructorId" value={formik.values.batchCoInstructorId} onChange={formik.handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 w-full">
                        ...
                        <div className="grid md:grid-cols-3 mt-7 gap-3">
                            {candidatesProgram.length == 0 ? <h1 className="text-center">Tidak ada kandidat</h1> : candidatesProgram.map((candidate: any) =>
                            <div key={candidate.prapUserEntityId} className={`inline-flex border gap-2 items-center justify-between border-slate-500 py-2 px-3 rounded-xl ${members.find((member: any) => member.idUser == candidate.prapUserEntityId) && 'bg-green-300 border-none'}`}>
                                ...
                                {members.find((member: any) => member.idUser == candidate.prapUserEntityId) ? 
                                    <button onClick={(e) => handleMemberButton(e, candidate.prapUserEntityId, 'remove')}>
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                                        </svg>
                                    </button> :
                                    <button onClick={(e) => handleMemberButton(e, candidate.prapUserEntityId, 'add')}>
                        ...
                        <div className="flex justify-end gap-2 mt-4">
                            ...
                            <button onClick={(e) => handleCancelButton(e)} className="bg-blue-500 text-white py-1 px-2 rounded-md hover:bg-blue-600">Cancel</button>
                        </div>
                    </form>
                </div>
            </Page>
