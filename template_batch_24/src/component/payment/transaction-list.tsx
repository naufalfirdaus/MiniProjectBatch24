...
export default function TransactionList() {
  ...
  const accountId = router.query.accountId ? (router.query.accountId as string) : '';

  useEffect(() => {

    setSearchQuery(accountId);

    dispatch(GetTransactionReq({ accountid: accountId, paymentType: selectedOption, page: currentPage, size: itemsPerPage }))
  }, [dispatch, refresh, accountId, selectedOption, currentPage])

  const handleSearch = () => {

    router.push(`/payment/accounts?accountId=${encodeURIComponent(searchQuery)}`)

    dispatch(GetTransactionReq({ accountId: searchQuery, paymentType: selectedOption }));
  }
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    dispatch(GetTransactionReq({ accountId: searchQuery, paymentType: selectedOption, pageno: page })); // Fetch data for the new page
  };
        ...
        <div className='flex flex-row text-black max-w-screen-lg py-12 h-full w-full'>
          ...
          <div className='mx-2'>
            <div className='my-4'>
              ...
              />
              <button className='bg-green-600 text-white mx-2 px-4 py-1.5 rounded-md'
                onClick={handleSearch}>Search</button>
              <select className='mx-2 px-4 py-1.5 rounded-md w-46 h-9' value={selectedOption} onChange={handleSelectChange}>
                ...
                <tbody>
                  {transactions?.data?.map((item: any, index: number) => (
                    <tr key={index} className='text-center'>
                      ...
                      <td className='px-6 py-3'>{moment(item.trpaModifiedDate).format("DD/MM/YYYY HH:mm")}</td>
                      ...
                      <td className='px-6 py-3'>{item.users ? (item.users.userFirstName + ' ' + item.users.userLastName) : '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
