...
export class TransactionsService {
  constructor(
    ...
    private serviceTrpa: Repository<TransactionPayment>,
  ...
  async verifyTransaction(accountNumber: string, req: any, fintech: any) {
    ...
    if (!isBalanceEnough) {
      throw new ForbiddenException(
        'Your balance is not enough for buy this item',
      );
    } else {
  ...
  private async verifyAccount(
    ...
    user: any,
  ): Promise<{ accountUser: UsersAccount }> {
    const accountUser = await this.serviceUsac.findOne({
      where: {
        usacAccountNumber: accountNumber,
        usacUserEntityId: user.UserId,
      },
    });

    if (!accountUser) {
      throw new NotFoundException(
        `Users Account with number ${accountNumber} is not found`,
      );
    }

    if (accountUser.usacStatus !== 'active') {
      throw new ForbiddenException(
        `Your users account with number ${accountNumber} is invalid`,
      );
    }
